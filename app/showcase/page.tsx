...
export default function ShowcasePage() {
  ...
  const active = useMemo(
    () => showcases.find((s) => s.id === activeId) ?? null,
    [showcases, activeId],
  )

  // Load the list of showcases on mount.
  useEffect(() => {
    listShowcases()
      .then((list) => {
        setShowcases(list)
      ...
      .catch(() => toast({ title: "Failed to load showcases", variant: "destructive" }))
  ...
  async function handleCreate() {
    ...
    try {
      ...
      setNewName("")
      toast({ title: "Showcase created", description: "Share it with the public link." })
    } catch {
      toast({ title: "Failed to create showcase", variant: "destructive" })
    } finally {
      setCreating(false)
    }
  }
  ...
  async function handleAddCard(card: InventoryItem) {
    ...
    if (active.items.some((i) => i.id === card.id)) {
      toast({ title: "Already in this showcase" })
      return
    }
    if (active.items.length >= SHOWCASE_CARD_LIMIT) {
      toast({
        ...
        description:
          "Raising this limit is a planned paid upgrade and isn't available yet.",
        variant: "destructive",
      })
      return
    }
  ...
  async function handleDeleteShowcase() {
    ...
    try {
      ...
      toast({ title: "Failed to delete showcase", variant: "destructive" })
    }
  }
  ...
  // IDs already in the active showcase — excluded from the picker results.
  const existingIds = useMemo(
    () => new Set((active?.items ?? []).map((i) => i.id)),
    [active],
  )
          ...
          <>
            ...
            <Card className="mb-6 border-blue-100/80 bg-white/88 shadow-sm backdrop-blur">
              ...
              <CardContent className="space-y-4 pt-6">
                ...
                {/* Add card — searchable picker (inventory or a binder) */}
                <ShowcaseCardPicker
                ...
                {atLimit && (
                  <p className="text-sm text-amber-600">
                    This showcase is at the {SHOWCASE_CARD_LIMIT}-card limit.
                    Going beyond {SHOWCASE_CARD_LIMIT} cards is a planned paid
                    upgrade and isn&apos;t available yet.
                  </p>
                )}
              </CardContent>
            </Card>
