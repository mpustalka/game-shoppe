...
export default function CustomerListsPage() {
  ...
  function addDraftItem(inventoryId: string) {
    ...
    if (draftItems.some((entry) => entry.id === item.id)) {
      toast({ title: "Already on the list" })
      return
    }
  ...
  async function handleSave() {
    ...
    try {
      ...
      resetDraft()
      toast({ title: "Customer list saved" })
    } catch (error) {
      toast({
  ...
  async function handleDelete(id: string) {
    ...
    try {
      ...
      setLists(previous)
      toast({ title: "Could not delete list", variant: "destructive" })
    }
  }
          ...
          <CardContent className="space-y-4">
            ...
            <div className="flex items-center justify-between border-t pt-3">
              <span className="text-sm text-muted-foreground">
                {draftItems.length} card{draftItems.length === 1 ? "" : "s"} ·{" "}
