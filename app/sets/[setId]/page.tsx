...
import {
  getCatalogCardsBySet,
  getCatalogSetById,
} from "@/lib/pokemon-catalog-server"
...
export async function generateMetadata({
  ...
  if (!set) {
    return {
      title:
        "Set Not Found - Team Rocket Markets",
    }
  }

  return {
    title:
      `${set.name} - Team Rocket Markets`,

    description:
      `Browse and add ${set.total} cards from ${set.name} to your inventory`,
  }
}
...
export default async function SetDetailPage({
  ...
  /*
   * Protect against malformed routes such as:
   *
   * /sets/undefined
   * /sets/null
   */
  ...
  /*
   * Both queries now hit our Supabase catalog.
   *
   * No PokemonTCG.io request happens here.
   */
  const [
    set,
    cards,
  ...
  /*
   * The importer stores every verified card count.
   *
   * Use the actual loaded card count when available so
   * special sets whose API metadata total differs from the
   * real card list still display sensibly.
   */
      ...
      <section className="relative overflow-hidden border-b border-white/10">
        ...
        <div className="relative mx-auto max-w-[1500px] px-4 py-6 sm:px-6 sm:py-9 lg:px-8">
          <Button
            ...
            variant="ghost"
            size="sm"
            className="-ml-2 mb-6 rounded-xl text-white/55 hover:bg-white/[0.06] hover:text-white"
          >
            <Link href="/sets">
          ...
          <div className="flex flex-col gap-7 md:flex-row md:items-center">
            ...
            <div className="min-w-0 flex-1">
              ...
              <p className="mt-2 text-lg font-medium text-white/45">
                {set.series}
              </p>
              ...
              <div className="mt-5 flex flex-wrap gap-2">
                ...
                <Badge
                  variant="outline"
                  className="border-emerald-400/15 bg-emerald-500/[0.06] text-emerald-300/70"
                >
