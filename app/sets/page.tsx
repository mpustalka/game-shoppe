...
export const metadata = {
  title: "Browse Pokemon Sets - Team Rocket Markets",
  description:
    "Browse Pokemon TCG sets and add cards to your Team Rocket Markets collection.",
}
...
export default async function SetsPage() {
  ...
  const sortedSeries = Object.entries(setsBySeries).sort((a, b) => {
    const newest = (seriesSets: typeof sets) =>
      Math.max(
        ...seriesSets.map((set) =>
          new Date(set.releaseDate).getTime(),
        ),
      )
          ...
          <div className="mt-5 flex flex-col gap-7 lg:flex-row lg:items-end lg:justify-between">
            <div className="max-w-3xl">
              ...
              <p className="mt-5 max-w-2xl text-sm leading-6 text-white/48 sm:text-base sm:leading-7">
                Explore {sets.length.toLocaleString()} English sets across{" "}
                {sortedSeries.length.toLocaleString()} series. Open a set to
                browse cards and add exact variants to your collection.
              </p>
            </div>
